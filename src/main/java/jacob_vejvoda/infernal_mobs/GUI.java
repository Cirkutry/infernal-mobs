...
public class GUI implements Listener {
	...
	private static void showBossBar(Player p, Entity e) {
		...
		String title = plugin.getConfig().getString("bossBarsName", "&fLevel <mobLevel> &fInfernal <mobName>");
		...
		if (!bossBars.containsKey(e)) {
			...
			try {
				bc = BarColor.valueOf(plugin.getConfig().getString("bossBarSettings.defaultColor", "WHITE"));
			} catch (IllegalArgumentException ignored) {
			}

			try {
				bs = BarStyle.valueOf(plugin.getConfig().getString("bossBarSettings.defaultStyle", "SOLID"));
			} catch (IllegalArgumentException ignored) {
			}
	...
	private static void fixScoreboard(Player player, Entity e, List<String> abilityList) {
		if (plugin.getConfig().getBoolean("enableScoreBoard") && (e instanceof Damageable)) {
			...
			if ((player.getScoreboard() == null) || (player.getScoreboard().getObjective(DisplaySlot.SIDEBAR) == null)
					...
					|| (!player.getScoreboard().getObjective(DisplaySlot.SIDEBAR).getName()
