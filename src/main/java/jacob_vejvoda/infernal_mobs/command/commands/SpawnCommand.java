...
public class SpawnCommand extends BaseCommand {
	...
	public boolean execute(CommandSender sender, String[] args) {
		...
		try {
			Location spoint = new Location(world, Integer.parseInt(args[3]), Integer.parseInt(args[4]),
					Integer.parseInt(args[5]));
			ArrayList<String> abList = new ArrayList<>(Arrays.asList(args).subList(6, args.length));

			if (spawn(sender, args[1], spoint, abList)) {
				sender.sendMessage(localeManager.getMessage("commands.spawn.success", args[1], args[2], args[3],
						args[4], args[5]));
