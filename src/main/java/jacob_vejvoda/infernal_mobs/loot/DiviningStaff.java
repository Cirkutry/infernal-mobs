...
public class DiviningStaff {
	...
	private ItemStack createRodFromConfig(String configPath, FileConfiguration config) {
		...
		try {
			...
			plugin.getLogger().warning("Invalid material '" + materialStr + "' in config at " + configPath);
	...
	public ItemStack getDiviningStaff() {
		...
		if (staff == null) {
			staff = getItem(Material.BLAZE_ROD, LootUtils.hex("&6&lDivining Rod"), 1,
	...
	public void addRecipe() {
		...
		if (plugin.getConfig().isConfigurationSection("diviningRod.recipe.ingredients")) {
			var ingredients = plugin.getConfig().getConfigurationSection("diviningRod.recipe.ingredients");
			if (ingredients != null) {
				for (String key1 : ingredients.getKeys(false)) {
					...
					if (materialName != null && !materialName.equals("AIR")) {
						try {
							...
							plugin.getLogger()
									.warning("Invalid material '" + materialName + "' in divining rod recipe");
						}
					}
				}
			}
	...
	private void beamParticles(Location loc) {
		...
		plugin.displayParticle(particle, loc.getWorld(), loc.getX(), loc.getY(), loc.getZ(), r, speed, amount);
	}
