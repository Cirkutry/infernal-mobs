...
public class PotionEffectHandler {
	...
	public void applyPotionEffects(LivingEntity entity, String effectID) {
		...
		for (Map<?, ?> potionConfig : potionEffects) {
			...
			entity.addPotionEffect(new PotionEffect(potionEffectType, duration, amplifier, ambient, particles, icon));
		}
	}
	...
	public void checkPlayerPotionEffects(Player player) {
		...
		for (String effectID : effectsSection.getKeys(false)) {
			ConfigurationSection effectSection = plugin.getLootFile()
					.getConfigurationSection("potionEffects." + effectID);
			if (effectSection == null)
				continue;
	...
	private boolean hasRequiredItemInSlots(Player player, ItemStack itemUsed, ItemStack requiredItem,
			List<String> slotStrings, boolean isCharm) {
	...
	private List<Integer> getSlotNumbers(String slotStr) {
		...
		try {
			if (slotStr.matches("\\d+")) {
				...
				plugin.getLogger()
						.warning("Invalid slot number: " + slotStr + ". Only numeric slot numbers are allowed.");
			}
