...
public class PSpawnCommand extends BaseCommand {
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        if (plugin.cSpawn(sender, args[1], p.getLocation(), abList)) {
            sender.sendMessage(localeManager.getMessage("commands.pspawn.success", args[1], p.getName()));
    ...
    public List<String> tabComplete(CommandSender sender, String[] args) {
        ...
        if (args.length == 3) {
            if (args[args.length - 1].isEmpty()) {
                ...
                newTab.addAll(CommandUtils.filterStartsWith(CommandUtils.getOnlinePlayerNames(), args[args.length - 1]));
            }
