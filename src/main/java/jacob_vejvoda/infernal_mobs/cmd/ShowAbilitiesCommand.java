    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        if (plugin.getTarget(player) != null) {
            ...
            if (plugin.idSearch(mobId) != -1) {
                ...
                if (!targeted.isDead()) {
                    sender.sendMessage(localeManager.getMessage("commands.showabilities.list", oldMobAbilityList.toString()));
                }
            } else {
                sender.sendMessage(localeManager.getMessage("commands.showabilities.not_infernal", targeted.getType().getName()));
            }
        } else {
