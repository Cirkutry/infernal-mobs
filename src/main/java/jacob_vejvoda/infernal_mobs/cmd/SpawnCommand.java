    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        try {
            if (EntityType.valueOf(args[1].toUpperCase()) != null) {
                ...
                if (args.length == 2) {
                    ...
                    for (int i = 2; i < args.length; i++) {
                        if (plugin.getConfig().getString(args[i]) != null) {
                            ...
                            sender.sendMessage(localeManager.getMessage("commands.spawn.invalid-ability", args[i]));
                            ent.remove();
                            return true;
                        }
                    }
                ...
                if (args.length > 2) {
                    sender.sendMessage(localeManager.getMessage("commands.spawn.success-with-abilities", args[1]));
                    sender.sendMessage(abList.toString());
                } else {
                    sender.sendMessage(localeManager.getMessage("commands.spawn.success-simple", args[1]));
                }
            } else {
